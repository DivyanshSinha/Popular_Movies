...
public class QueryUtils {
    ...
    private static List<movies> extractFeatureFromJson(String movieJSON) {
        ...
        try {
            ...
            // For each book in the movieArray, create an {@link movie} object
            for (int i = 0; i < movieArray.length(); i++) {
                ...
                if (currentMovie.has("poster_path")){
                    imagePath =  imagePath + currentMovie.getString("poster_path");
                }
                else {
                    Log.v("QueryUtils" , "image_path not found");
                }
                ...
                if (currentMovie.has("release_date")){
                    ...
                    Log.v("QueryUtils" , "release date not found");
                }

                String rating = "";
                if (currentMovie.has("vote_average")){
                    rating = currentMovie.getString("vote_average") + "/10";
                }
                else {
                    Log.v("QueryUtils" , "rating not found");
                }

                String description = "";
                if (currentMovie.has("overview")){
                    description = currentMovie.getString("overview");
                }
                else {
                    Log.v("QueryUtils" , "description not found");
                }

                // Create a new {@link movie} object with the title, id, image_path,
                // and url from the JSON response.
                movies movie = new movies(title , imagePath , id , releaseDate , rating , description);
